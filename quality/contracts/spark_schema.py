...
Silver parses the raw Debezium ``after``/``before`` JSON against this schema
in PERMISSIVE mode: a value that cannot be coerced becomes null, and the
contract's nullability rules then decide whether the row is a violation.
