...
Also snapshots the metrics Delta table to Parquet
(``<exports>/observability/pipeline_run_metrics``) so JVM-free consumers —
the Streamlit dashboard (duckdb) and the incident agent — can read metrics
...
def export_metrics_parquet(spark: SparkSession, config: LakehouseConfig) -> Path:
    out = config.storage.exports_dir / "observability" / "pipeline_run_metrics"
    (
        spark.read.format("delta")
        .load(str(metrics_table_path(config)))
        .coalesce(1)
...
def render_text(run_id: str, rows: list[Row]) -> str:
    lines = [
        ...
        "-" * 96,
        f"{'pipeline':10s} {'step':32s} {'status':8s} {'secs':>6s} "
    ...
    for r in rows:
        ...
        lines.append(
            f"{r['pipeline']:10s} {r['step']:32s} {r['status']:8s} "
            f"{r['duration_s']:6.1f} "
...
<html><head><meta charset="utf-8"><title>lakehouse run {html.escape(run_id)}</title>
<style>
...
def run_report(
    ...
    parquet_path = export_metrics_parquet(spark, config)
    print(f"\nhtml report -> {html_path}")
    return ReportPaths(html=html_path, metrics_parquet=parquet_path)
